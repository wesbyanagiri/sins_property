...
<section class="container mt-5" id="property">
    ...
    <?php
        if(!empty($_GET['prices']) && !empty($_GET['rooms'] && !empty($_GET['pools']) )):
    ...
    <div class="row mt-3">
        <div class="col-12">
            <p>Showing result with price up to $<?= (number_format($_GET['prices'], 2)) ?>, have
                ...
                <?= $_GET['pools'] == 'none' ? '0' : $_GET['pools'] ?> pools.</p>
        </div>
    </div>
    <?php
            else:
        ...
        <div class="col">
            <div data-aos="zoom-out-up" data-aos-delay="<?= $increment += 100 ?>" class="card">
                ...
                <div class="card-body">
                    ...
                    <p class="text-muted"><i class="fas fa-bed"></i> : <?= $filter['rooms'] ?></p>
                    <p class="text-muted"><i class="fas fa-swimmer"></i> :
                        <?= ($filter['pools'] == 'none') ? '0' : $filter['pools'];  ?></p>
                    <a href="<?= baseurl; ?>/agent/details/<?= $filter['id'] ?>" class="btn btn btn-outline-dark">See
                        Detail</a>
                </div>
            </div>
        </div>
